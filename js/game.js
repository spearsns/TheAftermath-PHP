...
$(document).ready(function(){
	...
	$('#LoSBtn').click(function(){
		...
		var array = [majorSuccess, directSuccess, minorSuccess, minorFail, directFail, majorFail];
		var length = array.length;
		for(i = 0; i < length; i++){
			sendChat(array[i], name);
		}
	...
	setInterval(getExp, 1250);
	var listing = setInterval(getPlayers, 1250);
	setInterval(getStoryteller, 1250);
	...
	$('.interface').on('click', '.awardExpBtn', function(e){ 
		...
		$.ajax({
			...
			success: 	function(response){
							...
							listing = setInterval(getPlayers, 1250);
